 ...
 *
 * The three-tap flow: name → drug page → Compare. A pick is a single push to
 * the drug page; the drug page's Compare link carries the drug through.
...
export function PsychSearch({ className }: { className?: string }) {
  ...
  function pick(name: string) {
    // Single push — no async fetch, no second navigation step.
    router.push(`/tools/psychopharm/${slugFor(name)}`);
  }
      ...
      {list.length > 0 ? (
        <>
          ...
          <ul
            ...
            className="absolute z-20 mt-1 max-h-72 w-full overflow-y-auto rounded-md border-2 border-border bg-card hard-shadow-sm"
          >
          {list.map((item, i) => (
            <li
              key={item}
