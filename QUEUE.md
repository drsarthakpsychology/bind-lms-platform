...
- [x] T190  Final product acceptance gate — the application is not complete
  ...
  - [ ] T191  AI simulation architecture audit — trace the complete patient
...
- [ ] T192  Remove accidental scripted execution — the configured patient
...
- [ ] T193  AI provider health check — verify the configured AI API is actually
...
- [ ] T194  End-to-end AI smoke test — create a test patient session that proves
...
- [ ] T195  Admin student-view AI parity — when an administrator opens "View as
...
- [ ] T196  Student/admin simulation parity — the same case configuration,
...
- [ ] T197  Explicit fallback architecture — if the real AI provider fails,
...
- [ ] T198  Patient case source of truth — create one structured source of
...
- [ ] T199  Patient behavioural profile — separate patient facts from patient
...
- [ ] T200  Dynamic patient engine — build the patient as an AI-driven
...
- [ ] T201  Patient system prompt architecture — create a robust server-side
...
- [ ] T202  Dynamic disclosure — the patient must not reveal the entire case
...
- [ ] T203  Behavioural variation — two students asking the same question may
  receive different natural wording and emotional responses while the
  underlying clinical facts remain consistent

- [ ] T204  Patient state machine — maintain structured state for emotional
...
- [ ] T205  Persistent conversation memory — preserve conversation state across
  every turn so the patient remembers what was already said and does not
  repeatedly reveal or forget the same information

- [ ] T206  Individual learner profile — create a student learning profile that
...
- [ ] T207  Personalised difficulty — adapt future practice to the individual
...
- [ ] T209  Learning memory boundaries — only store information genuinely
...
- [ ] T211  Learning signal extraction — after each activity identify useful
...
- [ ] T212  AI teaching adaptation — allow future explanations, practice
  recommendations, and simulation difficulty to use the student's actual
  learning profile

- [ ] T213  Text/voice single conversation state — text and voice must operate
  on the exact same underlying conversation; switching between them must never
  create a new session or lose context

- [ ] T214  Realtime voice architecture decision — evaluate the existing voice
...
- [ ] T215  Realtime WebRTC voice layer — implement a production-grade realtime
...
- [ ] T216  Realtime model integration — evaluate a true realtime speech-to-
...
- [ ] T217  Provider abstraction — do not hard-code the voice architecture to
...
- [ ] T218  Voice model evaluation — test the available realtime model options
...
- [ ] T219  Voice pipeline fallback — if a direct realtime speech-to-speech
...
- [ ] T228  Voice screen hierarchy — the voice screen should contain only the
...
- [ ] T229  Voice interaction model — design the interaction around
...
- [ ] T230  Voice/text transition — provide one obvious action to switch from
  voice mode to text mode without ending the patient session

- [ ] T231  Text transcript synchronisation — every spoken student input and
  AI response should become part of the same visible conversation transcript
  when transcription is available

- [ ] T232  Voice response transcript — while the AI is speaking, show the
  corresponding response text progressively where appropriate so the student
  can follow the content

- [ ] T233  Voice audio quality — implement proper audio capture, echo
...
- [ ] T235  Microphone permission UX — replace the large browser-specific
...
- [ ] T236  Voice failure recovery — if the realtime connection drops, reconnect
  without losing conversation state; if reconnect fails, allow immediate
  continuation through text

- [ ] T237  Voice session persistence — returning from voice mode must return
...
- [ ] T242  Voice emotional expression — where supported by the selected voice
