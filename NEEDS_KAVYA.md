...
- **LIVE MOBILE QA (11 QUEUE items, release-QA only)** — T77 workflow completion, T78 first-time, T79 returning, T80 interruption, T81 mobile regression matrix, T82 desktop regression, T83 visual comparison, T84 red-team, T88 polish, T89 end-to-end QA, T90 final acceptance. All need a running server (`npm run dev`) + a real browser/phone. The `e2e/mobile-matrix.spec.ts` matrix is committed and runnable once a server is up: `npx playwright test e2e/mobile-matrix.spec.ts`. Tick each after the live pass.
