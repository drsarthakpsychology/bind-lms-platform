...
All extraction to date is from **Stahl, Prescriber's Guide (7th ed.)** monographs. 
Cross-checking against Maudsley, Kaplan & Sadock, and Ahuja is a pending pass — 
conflict reconciliation (especially dose ranges) has NOT yet run. This is the honest 
gap: a single-source knowledge base is publishable (agreement: single) but must be 
flagged and cross-checked before two-source claims are made.

## Gaps (empty) — recorded, never invented

The following fields are NOT yet covered for most drugs and correctly read "Not covered in our sources":
- half_life / dose_form / interactions / monitoring / discontinuation (only in monographs with those headers)
- all comorbidity_notes, observation_prompts, session_observations (Phase 2 tables, empty until author) 
- special populations (renal/hepatic/elderly) per dose band
